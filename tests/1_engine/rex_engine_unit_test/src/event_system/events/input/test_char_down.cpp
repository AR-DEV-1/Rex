...
TEST_CASE("TEST - Events - Char Down Event")
{
  ...
  char8 key = 'a';
  auto subscription = rex::event_system::instance()->subscribe<rex::CharDown>([&num_quit_events_fired, &key](const rex::CharDown& evt)
    {
  ...
  rex::event_system::instance()->fire_event(rex::CharDown(key));
  REX_CHECK(num_quit_events_fired == 1);

  // Queued event fire
  key = 'b';
  rex::event_system::instance()->enqueue_event(rex::CharDown(key));
  REX_CHECK(num_quit_events_fired == 1);
