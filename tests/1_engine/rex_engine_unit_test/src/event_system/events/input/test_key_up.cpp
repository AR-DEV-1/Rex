...
TEST_CASE("TEST - Events - Key Up Event")
{
  ...
  auto subscription = rex::event_system::instance()->subscribe<rex::KeyUp>([&num_quit_events_fired, &key_code](const rex::KeyUp& evt)
    {
  ...
  rex::event_system::instance()->fire_event(rex::KeyUp(key_code));
  REX_CHECK(num_quit_events_fired == 1);

  // Queued event fire
  key_code = rex::KeyCode::B;
  rex::event_system::instance()->enqueue_event(rex::KeyUp(key_code));
  REX_CHECK(num_quit_events_fired == 1);
