	...
	namespace gfx
	{
		namespace shader_reflection_cache
		{
			...
			const ShaderPipelineReflection& load(const ShaderPipeline& pipeline)
			{
				...
				builder.process_shader(gfx::gal::instance()->reflect_shader(pipeline.vs), ShaderVisibility::Vertex);
				builder.process_shader(gfx::gal::instance()->reflect_shader(pipeline.ps), ShaderVisibility::Pixel);
