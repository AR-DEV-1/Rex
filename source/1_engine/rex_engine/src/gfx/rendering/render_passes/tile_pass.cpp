...
namespace rex
{
	namespace gfx
	{
		...
		void TilePass::reserve_objects()
		{}
		// Push a new tile to render
		void TilePass::push_object()
		{}
		...
		void TilePass::update_tileset_texture(const rex::gfx::Texture2D* texture)
		{}
		void TilePass::update_dimension(s32 width, s32 height)
		{}
		...
		s32 TilePass::num_tiles() const
		{
			return rendertarget_width_in_tiles() * rendertarget_height_in_tiles();
		}

		void TilePass::init(const TileRenderPassDesc& desc)
		{
		...
		void TilePass::init_tile_instance_to_tile_idx_buffer()
		{
		...
		void TilePass::init_map_matrix(const TileRenderPassDesc& desc)
		{
		...
		void TilePass::update_tile_data(const rsl::pointi8 cameraPos)
		{
			...
			for (s32 y = 0; y < num_tile_rows; ++y)
			{
				for (s32 x = 0; x < num_tile_columns; ++x)
				{
					...
					s32 block_idx = m_map_matrix.index_at_blockcoord(coord);
		...
		rsl::pointi8 TilePass::player_pos_to_screen_top_left(rsl::pointi8 coord)
		{
