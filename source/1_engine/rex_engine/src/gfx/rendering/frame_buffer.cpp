			...
			for (const auto& attachment_desc : desc)
			{
				...
				else
				{
					if (is_depth_format(attachment_desc.format()))
					{
						...
						m_depth_stencil_buffer = gfx::instance()->create_depth_stencil_buffer(attachment_desc.width(), attachment_desc.height(), attachment_desc.format(), attachment_desc.clear_state());
					}
					else
					{
						REX_ASSERT("To implement");
