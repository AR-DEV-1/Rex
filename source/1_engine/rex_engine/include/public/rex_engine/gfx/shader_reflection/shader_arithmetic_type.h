...
namespace rex
{
	namespace gfx
	{
		...
		public:
			...
			constexpr operator EScalarType() const
			{
				return m_value;
			}
			...
			bool operator==(ScalarType other) const
			{
		...
		// - float (32 bits)
		// - float (64 bits)
		class ShaderArithmeticType
		{
		public:
			enum EShaderArithmeticType
			{
				Unknown,
			...
			constexpr s32 component_count() const
			{
				switch (m_value)
				{
			...
			constexpr ScalarType scalar_type() const
			{
				switch (m_value)
				{
			...
			constexpr s32 format_byte_size() const
			{
				return component_count() * scalar_type().size();
			}

			// Return if the shader parameter type is normalized or not
			constexpr bool is_normalized_format() const
			{
				switch (m_value)
				{
				case rex::gfx::ShaderArithmeticType::UshortNorm:
			...
			constexpr bool is_convertible_to(ShaderArithmeticType to) const
			{
				if (m_value == to.m_value)
				{
					return true;
				}

				if (is_normalized_format() ^ to.is_normalized_format())
				{
					return true;
				}

				if (component_count() <= to.component_count())
				{
					return scalar_type() == to.scalar_type();
				}
