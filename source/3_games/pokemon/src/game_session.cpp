...
namespace pokemon
{
	...
	SaveFile GameSession::load_startup_savefile() const
	{
		...
		rsl::string startup_save_filepath(rex::cmdline::get_argument("StartupSaveFile").value_or(rex::path::join(rex::engine::instance()->project_root(), "startup_save_file.json")));
